...
steps = {
    "forward": {
        ...
        "cores": 16,
        "launcher_args": "-N adjoint_2d_cylindrical_forward -o forward.out -e forward.err -v PYOP3_SPMD_STRICT=1",
        "outputs": [
            "Checkpoint_State.h5",
            "forward.out",
            "forward.err",
...
for case, scheduler in itertools.product(cases, schedulers):
    ...
    steps[key] = {
        ...
        "cores": 16,
        "launcher_args": f"-m 64GB -N {key} -o {key}.out -e {key}.err -v PYOP3_SPMD_STRICT=1",
        "args": key,
